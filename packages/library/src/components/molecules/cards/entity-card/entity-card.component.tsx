...
export const EntityCard = ({
    ...
    const resolveSharingList = (): JSX.Element[] => {
        ...
        const connection = (connection: Owner) => {
            return (
                <UserImage
                    key={`user-image-entity-card-${connection.id}`}
    ...
    return (
        <div className={tw(S.EntityCardCss)}>
            <div className={tw(S.ButtonBoxCss)}>
                ...
                <Tooltip
                    content={`${
                        entity.visibility.isVisible ? 'Hide' : 'Show'
                    } this ${_isList ? 'list' : 'workshop'} ${
                        entity.visibility.isVisible ? 'from' : 'to'
                    } everyone`}
                    direction={DirectionType.LEFT}
                    size={SizeType.L}
                >
                    <Button
                        icon={{
                            icon: entity.visibility.isVisible
                                ? 'eye-on'
                                : 'eye-off',
            ...
            >
                <div className={tw(S.TitlesCss)}>
                    ...
                    {_isWorkshop && (
                        <h6>Number of lists: {entity.lists.length}</h6>
                    )}
                </div>
